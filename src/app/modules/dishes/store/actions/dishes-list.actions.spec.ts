...
import {
  ...
  dishesListLoad,
  dishesListSave,
  TypeActionsDisheslist,
} from './dishes-list.actions';

describe('shared Data Actions', () => {
  it('should dishesDataLoad', () => {
    expect(dishesListLoad()).toEqual({
      type: TypeActionsDisheslist.dishesListLoadString,
    });
  });
  it('should dishesDataSuccess', () => {
    const dishesDataSuccess: IDish[] = [
      {
        idMeal: '52977',
        strMeal: 'Corba',
    ...
    expect(dishesListSave(dishesDataSuccess)).toEqual({
      type: TypeActionsDisheslist.dishesListSaveString,
      dishes: dishesDataSuccess,
    });
  });
  it('should dishesDataError', () => {
    const vehicleDataError = {
      message: 'Internal server error',
    };
    expect(dishesListError(vehicleDataError)).toEqual({
      type: TypeActionsDisheslist.dishesListErrorString,
